 ...
 * the right shape, and none of them came off a datasheet. The point is not
 * accuracy, it is that the VCU cannot tell it is being lied to.
 */
#include "sim/plant.h"
...
void plant_set_brake_pct(plant_t *p, int pct_x10)
{
    if (pct_x10 < 0) pct_x10 = 0;
    if (pct_x10 > PCT_MAX) pct_x10 = PCT_MAX;
    p->brake_mv = (mv_t)(500 + (4000 * pct_x10) / PCT_MAX);
}
...
void plant_step(plant_t *p, const vcu_out_t *out, uint16_t dt_ms)
{
     ...
     * makes less heat, and the loop settles. Hold the pedal flat in SPORT
     * and it parks at about 94 C; ease off and it parks somewhere cooler.
     * Nobody wrote either number anywhere. It is just where the two curves
     * happen to meet. */
